...
if ENV["DOCKERIZED"] # Running is a docker container
  if ENV['SERVER_MODE'] == 'https' # HTTPS
    ssl_bind '0.0.0.0', ENV.fetch('SERVER_PORT', '443'), {
      key:  ENV.fetch('SSL_KEY_PATH', 'secrets/ssl.key'),
      cert: ENV.fetch('SSL_CERT_PATH', 'secrets/ssl.crt')
    }
  else
    bind "tcp://0.0.0.0:#{ENV.fetch('SERVER_PORT', '80')}"
  end
elsif ENV["RAILS_ENV"] == "production"
  # NGINX is used to forward 443 to 8443 and provide certification and SSL.
  bind "tcp://0.0.0.0:#{ENV.fetch('SERVER_PORT', '8443')}"
else
  # Dev mode
  bind "tcp://0.0.0.0:#{ENV.fetch('SERVER_PORT', '3000')}"
end
