...
const HtmlEditor = ({ id = "", value = "", placeholder = "Enter text here...", onChange = null, onBlur = null, useHtmlView = false, noSwitchButton = false, }) => {
    ...
    useEffect(() => {
        if (!window.htmlEditorStates)
            window.htmlEditorStates = {};
    ...
    function prettyPrintHtml(html) {
        ...
        formattedHtml = formattedHtml.replace(new RegExp(`(<(${blockTags.join("|")})(\\s[^>]*)?>)`, "g"), "\n  $1\n");
        formattedHtml = formattedHtml.replace(new RegExp(`(</(${blockTags.join("|")})>)`, "g"), "\n$1\n");
    ...
    return (_jsxs("div", { children: [isHtmlView && !noSwitchButton ? (_jsx("textarea", { id: `${id}_text`, value: editorContent, onChange: (e) => setEditorContent(e.target.value), onBlur: handleBlur, className: "form-control", placeholder: "Edit raw HTML here", rows: 10 })) : (_jsx(ReactQuill, { ref: quillRef, theme: "snow", id: id, value: editorContent, onChange: handleChange, onBlur: handleBlur, modules: { toolbar: toolbarOptions }, className: "form-control", placeholder: placeholder })), !noSwitchButton && (_jsx("div", { className: "row align-items-center", children: _jsxs("div", { className: "flex-container", children: [isHtmlView ? (_jsx("button", { type: "button", onClick: toggleView, className: "btn btn-good mt-2 mb-2", children: "Switch to Editor View" })) : (_jsx("button", { type: "button", onClick: toggleView, className: "btn btn-bad mt-2 mb-2", children: "Switch to HTML View **" })), _jsx("span", { className: "ms-4 mt-2", children: "** HTML View should only be used by users who are familiar with HTML" })] }) }))] }));
};
